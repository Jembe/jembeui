...
class CFormBase(Component):
    ...
    Override get_form_type to return subclass of jembeui.Form that
    ...
    def form(self) -> "Form":
        ...
        if self._config.form_state_name in self.state.keys():
            ...
            self.validate_form_instance(self.state.form)
            ...
            self.validate_form_instance(self._form)
    ...
    def validate_form_instance(self, form: "jembeui.Form"):
        ...
        for field in form:
            if isinstance(field, FieldMixin):
                raise JembeUIError(
                    f"{self.__class__.name}(CFormBase) doesn't support "
                    "jembeui fields that extends jembeui.FieldMixin!"
                )
