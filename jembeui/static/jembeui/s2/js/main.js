...
function jembeui_s2_init_mui(el) {
  if (el.classList.contains('mdc-snackbar')) {
    ...
    el.classList.contains('mdc-drawer')
    && el.classList.contains('mdc-drawer--modal')
    && el._mdcDrawer === undefined
    && el.classList.contains('mdc-drawer--modal')
  ) {
    // Modal drawer
    el._mdcDrawer = mdc.drawer.MDCDrawer.attachTo(el);
  } else if (
    el.classList.contains('mdc-top-app-bar')
    && el._mdcTopAppBar === undefined
  ) {
    // Top App Bar
